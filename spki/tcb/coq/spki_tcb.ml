(** Spki_tcb - Bridge module for Rocq extraction
    ...
    Type mapping:
      Rocq bytes (list nat) = int list in OCaml
...
(** Convert int list (Rocq bytes) to native bytes *)
...
(** Convert native bytes to int list (Rocq bytes) *)
...
(** Constant-time comparison for Rocq bytes *)
