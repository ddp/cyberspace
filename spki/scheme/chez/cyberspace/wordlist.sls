...
(library (cyberspace wordlist)
  ...
  (define (fips181-encode data)
    ...
    (let ((bytes (if (bytevector? data) data (string->utf8 data))))
      (bytes->fips181 bytes)))
  ...
  (define (pubkey->words pubkey)
    ...
    (let* ((hash (sha256-hash pubkey))
           (bytes hash))
      (list (byte->word (bytevector-u8-ref bytes 0) 0)
            (byte->word (bytevector-u8-ref bytes 1) 1)
            (byte->word (bytevector-u8-ref bytes 2) 2)
            (byte->word (bytevector-u8-ref bytes 3) 3))))
