...
(define (test-case name thunk)
  (display (sprintf "Testing ~a... " name))
  (thunk)
  (print "✓"))
...
(print "=== SPKI Certificate Tests ===\n")

;;; Test 1: Create principals
(test-case "create key principal"
  (lambda ()
    (let ((public-key (make-u8vector 32 0)))
...
(test-case "create validity"
  (lambda ()
    (let ((validity (make-validity "2026-01-01T00:00:00Z" "2027-01-01T00:00:00Z")))
      (assert-true (validity? validity) "should be validity")
      (assert-equal "2026-01-01T00:00:00Z" (validity-not-before validity) "not-before")
      (assert-equal "2027-01-01T00:00:00Z" (validity-not-after validity) "not-after"))))

;;; Test 5: Create certificate
(test-case "create certificate"
  (lambda ()
    (let* ((alice-pk (make-u8vector 32 1))
           (bob-pk (make-u8vector 32 2))
...
(test-case "principal s-expression round-trip"
  (lambda ()
    (let* ((public-key (make-u8vector 32 42))
...
(test-case "validity s-expression round-trip"
  (lambda ()
    (let* ((validity (make-validity "2026-01-01T00:00:00Z" "2027-01-01T00:00:00Z"))
      ...
      (assert-equal "2026-01-01T00:00:00Z" (validity-not-before validity2) "not-before")
      (assert-equal "2027-01-01T00:00:00Z" (validity-not-after validity2) "not-after"))))
  ...
  (lambda ()
    ...
    (let* ((keypair (ed25519-keypair))
      ...
      (let* ((issuer (make-key-principal public-key))
             (subject (make-key-principal public-key))  ; self-signed
  ...
  (lambda ()
    ...
    (let* ((keypair (ed25519-keypair))
      ...
      (let* ((issuer (make-key-principal public-key))
        ...
        ;; Tamper with certificate (change tag)
...
(print "Certificate module is working correctly.")
(print "Ready to implement CLI tools.")
