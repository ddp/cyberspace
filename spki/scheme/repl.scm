...
(define (rebuild-module! module arch stamp)
  ...
  (let* ((paths (libsodium-paths arch))
    ...
    (let* ((strict-exempt '("vault"))
           ...
           ;; -L<path> adds search path, -L -l<name> passes -l<name> to linker
           (lib-flags " -L/opt/homebrew/lib -L/usr/local/lib -L -lsodium -L -lkeccak")
    ...
    (else
     ...
     (if (eq? *user-mode* 'novice)
         ...
         (begin
           ...
           (print "  seek s          - Search across soup, audit, wormhole")
           (print "")
             ...
             (handle-exceptions exn
               ...
               (let ((result
                      (cond
                        ...
                        ((and (pair? expr) (eq? (car expr) 'seek))
                         (if (null? (cdr expr))
                             (print "Usage: (seek \"term\")")
                             (seek (cadr expr))))
