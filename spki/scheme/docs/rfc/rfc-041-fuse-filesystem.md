...
(define cap:read-only
  '(read stat readdir xattr-read acl-read))

(define cap:read-write
...
```scheme
(define (wormhole-open fs-path #!key (vault-path "/") (cert #f))
  "Open wormhole - requires valid certificate"
  (unless cert
    (error 'unauthorized "Wormhole requires SPKI certificate"))
  (unless (verify-wormhole-cert cert fs-path vault-path)
    (error 'unauthorized "Invalid wormhole certificate"))
  ;; Audit the open
  (wormhole-audit 'wormhole-open fs-path `(vault ,vault-path))
