...
Both formats can coexist. The manifest `format` field determines restoration method:
- `(format cryptographic)` → `seal-restore-cryptographic`
- `(format zstd-age)` → `seal-restore-zstd-age`

---

## References

1. [Zstd](https://github.com/facebook/zstd) - Facebook's compression algorithm
2. [Age](https://age-encryption.org/) - Modern encryption tool
3. [RFC-006](rfc-006-vault-architecture.md) - Vault System Architecture
4. [RFC-004](rfc-004-spki-authorization.md) - SPKI Authorization
...
**Compression:** Zstd (parallel)
**Encryption:** Age (X25519 + ChaCha20-Poly1305)
**Signature:** Ed25519 (SPKI)
