...
(rfc
  ...
  (section
    ...
    (p "- Expressive - Complex predicates, joins across objects - Efficient - Indexes, bloom filters, query planning - Secure - Queries respect capability boundaries - Auditable - Query patterns reveal access patterns")
  ...
  (section
    ...
    (subsection
      ...
      (p "- Cardinality estimation: How many rows will each predicate return? - Index selectivity: How discriminating is each index? - I/O cost: Sequential scan vs. random access - Join ordering: Which order minimizes intermediate result sizes?")
  ...
  (section
    ...
    (subsection
      "Performance"
      (p "- Index selection is critical for large soups - Bloom filters for fast negative lookups - Query result streaming to avoid memory exhaustion - Connection pooling for federated queries"))
    (subsection
      "Compatibility"
      (p "- Query language inspired by Newton's soup cursors - SQL-like semantics where applicable - S-expression syntax for Scheme integration")))
  (section
    "References"
  ...
  (section
    "Changelog"
    (p "- 2026-01-09 - Cost-based query optimization: Selinger algorithm, cardinality estimation, histograms, multi-tier caching with generation-aware TTL - 2026-01-07 - Initial draft")))
