...
#   .pdf  - PDF (derived from PostScript via Ghostscript)
...
// Platform-aware format picker - be kind to the reader
function go(base) {
  var p = navigator.platform || '';
  var ext = /Mac|iPhone|iPad|iPod/.test(p) ? '.pdf'
          : /Linux|X11|FreeBSD|OpenBSD/.test(p) ? '.ps'
          : '.pdf';  // Windows, unknown → PDF
  location.href = base + ext;
  return false;
}
  </script>
</head>
<body>
  ...
  <h2>Memos</h2>
  <input type="text" id="memo-search" placeholder="Filter memos..." autocomplete="off" spellcheck="false"
         style="width:100%;max-width:40em;padding:0.3em 0.5em;margin-bottom:0.5em;font-family:inherit;font-size:inherit;background:var(--bg);color:var(--fg);border:1px solid var(--border-dim)">
  <table>
    <thead>
      <tr>
        <th>Memo</th>
...
HEADER
  ...
  for memo in "${MEMOS[@]}"; do
    ...
    if is_reserved "$memo"; then
      ...
      numlink='<a href="'"${memo}"'.pdf" onclick="return go('"'"''"${memo}"''"'"')" title="Smart format (PDF/PS based on platform)">'${num}'</a>'
    fi
...
EOF
  ...
  <details class="platform-help">
    <summary>Viewing tips by platform</summary>
    <p>For pixel-perfect box diagrams, use PostScript or PDF instead of HTML.</p>
    <h4>macOS</h4>
    <pre><code>open memo-0002-architecture.ps    # TeXShop or Preview
open memo-0002-architecture.pdf   # Preview</code></pre>
    <h4>Linux / BSD</h4>
    <pre><code>gv memo-0002-architecture.ps      # Ghostview
evince memo-0002-architecture.pdf # GNOME
zathura memo-0002-architecture.pdf
gs -dNOPAUSE -dBATCH memo-0002-architecture.ps  # raw</code></pre>
    <h4>Windows</h4>
    <pre><code>start memo-0002-architecture.pdf  # Default PDF viewer
:: PostScript: install Ghostscript, then gsview</code></pre>
  </details>

  <footer>
    <p><a href="../../../../directory.html">Research Library</a></p>
    <p>S-expression source. Text: immortal. PostScript: open. PDF: portable. HTML: convenient.</p>
  </footer>
  <script>
...
chmod 644 *.html *.ps *.pdf *.txt *.css 2>/dev/null || true
...
if /usr/bin/ssh -q -o BatchMode=yes -o ConnectTimeout=5 "$YOYODYNE_HOST" exit 2>/dev/null; then
  ...
  rsync -av --delete --chmod=F644,D755 *.html *.ps *.pdf *.txt *.css *.woff2 *.svg "$YOYODYNE_HOST:$YOYODYNE_MEMO_PATH"
