  ...
  (define (compute-scaling-factor members)
    ...
    (if (null? members)
        ...
        (let* (;; Score all members
          ...
          `((reference . ,ref-name)
            (reference-score . ,(exact->inexact ref-score))
            (members . ,(map (lambda (n)
                               (cons (car n) (exact->inexact (cdr n))))
                             normalized))
            (aggregate-score . ,(exact->inexact aggregate))
            (member-count . ,count)
            (effective-capacity . ,(exact->inexact effective))
            (recommended-replication . ,(min count 3))))))

) ;; end module

;;;
;;; Example: Automated master election (prefer !mobile)
;;;
;;;   (import capability enroll)
;;;
;;;   ;; Gather hardware from discovered peers
;;;   ;; Note: introspect-hardware now includes (mobile #t/#f)
;;;   (define starlight-hw (introspect-hardware))  ; MacBook -> mobile: #t
;;;   (define fluffy-hw (introspect-hardware))     ; Mac Studio -> mobile: #f
;;;
...
;;;       (fluffy . ,fluffy-hw)))
;;;
;;;   ;; Most capable wins (mobile devices heavily penalized)
;;;   (let-values (((winner score all) (elect-master candidates)))
;;;     (printf "Winner: ~a with score ~a~n" winner score)
;;;     (printf "All scores: ~a~n" all))
;;;   => Winner: fluffy with score 520.0
;;;      All scores: ((starlight . 16.8) (fluffy . 520.0))
;;;
;;; Example: Dynamic scaling factor (normalized to capable)
;;;
;;;   (import capability)
;;;
;;;   (define members
...
;;;       (fluffy . ,fluffy-hw)))
;;;
;;;   (compute-scaling-factor members)
;;;   => ((reference . fluffy)
;;;       (reference-score . 520.0)
;;;       (members . ((fluffy . 1.0) (starlight . 0.032)))
;;;       (aggregate-score . 536.8)
...
;;;
;;;   ;; Interpretation:
;;;   ;; - fluffy is the reference (scale 1.0)
;;;   ;; - starlight contributes only 3.2% of fluffy's capacity
;;;   ;; - Total effective capacity is 1.032 "fluffy units"
;;;   ;; - Work distribution: fluffy gets ~97%, starlight gets ~3%
;;;
