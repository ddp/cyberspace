;;; filetype.scm - File Type Inspector
;;;
...
;;;
;;; Reference: /usr/share/file/magic/
...
(module filetype
  (;; Primary interface
   ...
   magic-type         ; blob/string -> symbol or #f
   magic-mime         ; blob/string -> mime-type or #f
  ...
  (define (read-magic-bytes source n)
    "Read first n bytes from source (path, blob, or string).
     Returns u8vector or #f on error."
                     ...
                     (if (>= i n)
                         ...
                         (let ((b (read-byte port)))
                           (if (eof-object? b)
                               buf
                               (begin
                                 (u8vector-set! buf i b)
                                 (loop (+ i 1))))))))))
             ;; It's raw bytes as a string
             (let ((buf (make-u8vector (min n (string-length source)) 0)))
               (let loop ((i 0))
                 (if (>= i (u8vector-length buf))
                     buf
                     (begin
                       (u8vector-set! buf i (char->integer (string-ref source i)))
                       (loop (+ i 1))))))))
        ((blob? source)
         (let* ((vec (blob->u8vector source))
                (len (min n (u8vector-length vec)))
                (buf (make-u8vector len 0)))
           (let loop ((i 0))
             (if (>= i len)
                 buf
                 (begin
                   (u8vector-set! buf i (u8vector-ref vec i))
                   (loop (+ i 1)))))))
        ((u8vector? source)
         source)
        (else #f))))
    ...
    (let ((offset (caddr pattern))
      ...
      (let loop ((i 0) (m magic))
        (cond
          ...
          ((not (= (u8vector-ref bytes (+ offset i)) (car m))) #f)
          (else (loop (+ i 1) (cdr m)))))))
  ...
  (define (wav? source)
    ...
    (let ((bytes (read-magic-bytes source 12)))
      (and bytes
           (>= (u8vector-length bytes) 12)
           (= (u8vector-ref bytes 0) #x52)   ; R
           (= (u8vector-ref bytes 1) #x49)   ; I
           (= (u8vector-ref bytes 2) #x46)   ; F
           (= (u8vector-ref bytes 3) #x46)   ; F
           (= (u8vector-ref bytes 8) #x57)   ; W
           (= (u8vector-ref bytes 9) #x41)   ; A
           (= (u8vector-ref bytes 10) #x56)  ; V
           (= (u8vector-ref bytes 11) #x45)))) ; E
  ...
  (define (xml? source)
    ...
    (let ((bytes (read-magic-bytes source 50)))
      (and bytes
           (>= (u8vector-length bytes) 5)
           (or ;; <?xml
               (and (= (u8vector-ref bytes 0) #x3c)
                    (= (u8vector-ref bytes 1) #x3f)
                    (= (u8vector-ref bytes 2) #x78)
                    (= (u8vector-ref bytes 3) #x6d)
                    (= (u8vector-ref bytes 4) #x6c))
               ;; UTF-8 BOM + <?xml
               (and (>= (u8vector-length bytes) 8)
                    (= (u8vector-ref bytes 0) #xef)
                    (= (u8vector-ref bytes 1) #xbb)
                    (= (u8vector-ref bytes 2) #xbf)
                    (= (u8vector-ref bytes 3) #x3c)
                    (= (u8vector-ref bytes 4) #x3f))))))
  ...
  (define (html? source)
    ...
    (let ((bytes (read-magic-bytes source 100)))
      (and bytes
           ...
           (let ((s (list->string (map integer->char
                                       (u8vector->list bytes)))))
  ...
  (define (json? source)
    ...
    (let ((bytes (read-magic-bytes source 100)))
      (and bytes
           (> (u8vector-length bytes) 0)
           (looks-like-text? bytes)
           (let ((first (u8vector-ref bytes 0)))
             (or (= first #x7b)    ; {
                 (= first #x5b)))))) ; [
  ...
  (define (file-command path flag)
    ...
    (handle-exceptions exn #f
      (let ((result (with-input-from-pipe
                        (sprintf "file ~a ~a 2>/dev/null" flag
                                (sprintf "'~a'" path))
                        read-line)))
  ...
  ;; Note: string-contains-ci is provided by srfi-13
