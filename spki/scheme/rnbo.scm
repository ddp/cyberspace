;;; rnbo.scm - Cycling '74 Max/RNBO Bridge for Cyberspace
;;;
...
(import osc)
(import (only vault lamport-time lamport-tick!))
...
(define (rnbo-connect #!optional (host "127.0.0.1") (port 7400))
...
(define (stream-gossip event-type #!optional (peer "") (data 0))
  "Stream gossip protocol event.
   event-type: 'announce | 'sync | 'ack | 'nack | 'heartbeat
   peer: peer identifier (string)
   data: optional numeric payload"
...
(define (stream-lamport #!optional (tick #f))
  "Stream Lamport clock tick.
   If tick is #f, uses current lamport-time.
   Also sends tick as normalized float (0.0-1.0) for audio."
  (let* ((t (or tick (lamport-time)))
...
(define (stream-object op hash #!optional (size 0))
...
(define (stream-key op #!optional (key-type 'ed25519) (bits 256))
  "Stream key operation.
   op: 'generate | 'sign | 'verify | 'derive | 'load
   key-type: ed25519 | x25519 | dilithium
   bits: key size (for logging)"
  (let ((op-num (case op
                  ((generate) 1)
                  ((sign) 2)
                  ((verify) 3)
...
(define (stream-sync state #!optional (peers 0) (behind 0))
  "Stream sync state change.
   state: 'idle | 'syncing | 'complete | 'error
   peers: number of connected peers
   behind: number of objects behind (if syncing)"
...
(define (start-heartbeat #!optional (interval-ms 100))
...
;;;
;;;   (import rnbo)
;;;   (rnbo-connect)                    ; default localhost:7400
;;;   (stream-lamport 42)               ; send lamport tick
;;;   (stream-gossip 'sync "peer1" 100) ; gossip event
;;;   (rnbo-disconnect)
;;;
;;; Example: Vault monitoring
;;;
;;;   (import rnbo vault)
;;;   (rnbo-connect "192.168.1.50" 7400) ; remote Max machine
;;;   (stream-vault-state
;;;     (vault-object-count)
;;;     (vault-key-count)
;;;     (vault-audit-count))
;;;
;;; Example: Hardware sonification
;;;
;;;   (import rnbo enroll)
;;;   (rnbo-connect)
;;;   (stream-introspection (introspect-hardware))
;;;   ; Max receives: /cyberspace/hw/cores, /hw/memory, etc.
;;;
;;; Max/MSP Patch Notes:
;;;
;;;   Use [udpreceive 7400] to receive messages.
;;;   Route with [route /cyberspace] then sub-routes.
;;;
...
;;;   [route /cyberspace]
;;;   |
;;;   [route /gossip /lamport /object /key /sync /vault /hw /heartbeat]
;;;   |
;;;   [... your audio/visual processing ...]
;;;
